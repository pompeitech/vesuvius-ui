...
**Current release: `0.5.0` — atoms, plus twelve molecules.**

| Tier | Status | Components |
| --- | --- | --- |
| **Atoms** | ✅ Shipped in `0.1.0` | 20 — see the list below |
| **Molecules** | 🚧 Shipping a few at a time | `Select`, `List` shipped in `0.2.0`; `Accordion`, `Popover`, `Tabs`, `Tooltip` shipped in `0.3.0`; `Breadcrumb`, `Toggle`, `ToggleGroup` shipped in `0.4.0`; `Dialog`, `DropdownMenu`, `AlertDialog` shipped in `0.5.0`; Combobox, DataTable's smaller siblings, and more still to come |
...
### Molecules shipped in `0.2.0` – `0.5.0`
...
on their respective Radix UI primitive) in `0.5.0`.
...
snapshots. `0.5.0` ships 32 test files — one per component — all green in CI on every push and pull
request.
